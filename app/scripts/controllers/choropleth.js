...
app.controller('ChoroplethCtrl', function ($scope){
...
var nation = crossfilter(),
  all = nation.groupAll(),
  per_cap = nation.dimension(function(d) { return d.ems_mean; }),
  per_caps = per_cap.group(),
  population = nation.dimension(function(d) { return d.pov; }),
  populations = population.group();

queue()
    .defer(d3.json, 'counties.json')
    .defer(d3.csv, 'choropleth.csv', function(d) {
      ...
      rateById.set(d.id, d.ems_mean);
      popById.set(d.id, d.pov);
  ...
  function barChart(percent) {
    ...
    brush.on('brush.chart', function() {
      ...
      svg.attr('class', 'counties')
        .selectAll('path')
          .attr('class', function(d) { if (selected.indexOf(d.id) >= 0) {return 'q8-11'} else if (extant.indexOf(d.id) >= 0) {return 'q5-11'} else {return null;}});
